...
- **Two link modes** — Hardware ACK (reliable, ~50 kbps) or Software Flow Control (faster, ~80–100 kbps, **default**)
- **Cooperative half-duplex** — automatic TX yield token in SWFLOW mode eliminates collisions during simultaneous bidirectional transfers
...
| `ATSHWACK=n` | ACK mode: `0`=software flow control (**default**)  `1`=hardware ACK |
...
> **Note:** `ATSHWACK` defaults to `0` (SWFLOW). Cooperative half-duplex is only available in SWFLOW mode.
...
### SWFLOW mode (`ATSHWACK=0`, **default**)

Auto-ACK disabled. Packets fly continuously up to a 4-slot retransmit window. The receiver sends `PKT_NACK` on gaps and `PKT_SWACK` (cumulative ACK) on each in-order packet. A 150 ms retransmit timer catches lost packets when both data and NACK are dropped. Throughput ~80–100 kbps on a clean link.

SWFLOW mode also enables **cooperative half-duplex** (see below) — the recommended mode for any bidirectional data transfer.

### HWACK mode (`ATSHWACK=1`)

Hardware acknowledgement via the nRF24L01's built-in auto-ACK and retry mechanism. Every 32-byte packet is confirmed before the next is sent. Reliable on noisy links; throughput limited to ~50 kbps by the ACK turnaround. Cooperative duplex is **not available** in HWACK mode — the hardware ACK carries no payload and cannot signal a yield.

**Maximum throughput settings:**
```
ATSSPEED=2        (2 Mbps air rate)
ATSHWACK=0        (software flow control — default)
...
In SWFLOW mode the modem uses a **TX yield token** piggybacked onto every software ACK packet to eliminate collisions automatically:

- When side B receives a data packet from side A and has **nothing to send**, it replies with a plain `PKT_SWACK`. Side A continues transmitting normally.
- When side B receives a data packet and **has data queued**, it replies with `PKT_SWACK_YIELD`. Side A sees the yield flag and skips one TX turn, giving side B an uncontested window to transmit.
- Side B then sends its packet and ACKs back — if side A still has data, it includes a yield flag too, handing the token back to A.

The result is automatic packet-by-packet role alternation under full duplex load:

```
Both sides have data:

A ──[data seq=5]──────────────────────────────► B
A ◄──────────────[SWACK_YIELD seq=5]──────────  B  (B has data → yield)
A  (skips TX turn)
B ──[data seq=0]──────────────────────────────► A
B ◄──────────────[SWACK_YIELD seq=0]──────────  A  (A has data → yield back)
A ──[data seq=6]──────────────────────────────► B
... alternates until one side's buffer drains
```

This requires no configuration — it is always active in SWFLOW mode. Under one-way traffic there is zero overhead: plain `PKT_SWACK` is sent and the yield path is never taken.

---

## Keep-Alive

When `S10=1`, the **dialling side** (initiator) sends a `PKT_PING` every S11 seconds. The **answering side** always replies with `PKT_PONG` regardless of its own S10 setting.
...
| PKT_SWACK | 0x08 | SWFLOW: cumulative acknowledgement |
...
| **Poor duplex throughput / data loss both ways** | Using HWACK mode, no collision avoidance | Switch to SWFLOW (`ATSHWACK=0`, now default) |
